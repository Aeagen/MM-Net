...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size):
        ...
        self.hybrid_model = CNNEncoder(config, n_channels=4)
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        encoded, attn_weights = self.encoder(embedding_output)  # (B, n_patch, hidden) (1,5,5,6,252)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        x = self.up(x)
        if skip is not None:
            print("x:{},skip:{}".format(x.shape, skip.shape))
            # skip = skip.permute(0, 1, 2, 4, 3)
            # print("x:{},skip:{}".format(x.shape, skip.shape))
