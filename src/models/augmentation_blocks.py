...
class DataAugmenter(nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():
            if random() < self.p:
                shift = (torch.from_numpy(np.random.uniform(-0.1, 0.1, size=[1, x.shape[1], 1, x.shape[3], 1])).float()).to(x.device)

                x = x * uniform(0.9, 1.1) + shift
                ...
                noise = (torch.from_numpy(np.exp(0.1*torch.randn(x[0,0].shape, dtype=torch.float32).numpy()))).to(x.device)

                # noise = torch.stack([torch.normal(a, std * 0.1) for std in std_per_channel]
                #                     ).to(x.device)

                # noise = torch.stack([torch.normal(mean=0., std=std * 0.1, out=x[0, 0].shape) for std in std_per_channel]
                #                     ).to(x.device)
                x = x + noise
