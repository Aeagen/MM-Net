...
def pad_or_crop_image_valid(image, seg=None, target_size=(128, 144, 144)):
    c, z, y, x = image.shape
    ...
    todos1 = get_left_right_idx_should_pad_valid_z(target_size[0], z)
    todos2 = get_left_right_idx_should_pad_valid(target_size[1], y)
    todos3 = get_left_right_idx_should_pad_valid(target_size[2], x)
    todos = [todos1, todos2, todos3]

    # todos = [get_left_right_idx_should_pad_valid(size, dim) for size, dim in zip(target_size, [z, y, x])]
    ...
    if seg is not None:
        ...
        return image, seg
    return image
def get_left_right_idx_should_pad_valid(target_size, dim):
    if dim >= target_size:
        return [False]
...
def get_left_right_idx_should_pad_valid_z(target_size, dim):
    if dim >= target_size:
        return [False]
    elif dim < target_size:
        pad_extent = target_size - dim
        left = 2
        right = pad_extent - left  # 3
        return True, left, right
def pad(img, pad_size=(0, 5, 16, 16)):
    px = tuple(zip([0]*len(pad_size), pad_size))
    dim = len(img.shape)
    return np.pad(img, px[:dim], mode='constant')
