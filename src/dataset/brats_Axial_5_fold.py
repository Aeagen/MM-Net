...
from src.dataset.image_utils import pad_or_crop_image, irm_min_max_preprocess, zscore_normalise, pad
from cropPatch.crop import crop_pad
...
class Brats(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # patient_image = {key: self.load_nii(_patient[key]) for key in _patient if key not in ["id"]}
        ...
        if self.training:
            ...
            # patient_image, patient_label = pad_or_crop_image_valid(patient_image, patient_label, target_size=(160, 256, 256))

            # print("patent---", patient_image.shape)
            # patient_label = crop_pad(patient_label)
            # patient_label = patient_label.astype("float16")
            # patient_label = torch.from_numpy(patient_label).permute(1, 0, 2, 3, 4)
            patient_image = crop_pad(patient_image, crop_size=(128, 128, 128))
            patient_image = patient_image.astype("float16")
            patient_image = torch.from_numpy(patient_image).permute(1, 0, 2, 3, 4)
            # print("patient",patient_image.shape)
        zmin = zmin; zmax = zmax
        ymin = ymin; ymax = ymax
        xmin = xmin; xmax = xmax
        # print("zmin111:{},zmax111:{}".format(zmin, zmax))
...
def get_datasets(seed, debug, no_seg=False, on="train", full=False,
    ...
    if full:
        if True:
            on = "train_val"
            base_folder_val = pathlib.Path(get_brats_folder(on)).resolve()
            assert base_folder_val.exists()
            patients_dir_val = sorted([x for x in base_folder_val.iterdir() if x.is_dir()])
        len_data_val = len(patients_dir_val)
        train_data = patients_dir[0:len_data_val - 40]
        val_data = patients_dir_val[len_data_val - 40:len_data_val]

        if full:
            train_dataset = Brats(train_data, training=True, debug=debug,
                                  normalisation=normalisation)
            val_dataset = Brats(val_data, training=False, data_aug=False,  debug=debug,
                            normalisation=normalisation)
    ...
    bench_dataset = Brats(patients_dir, training=False, benchmarking=True, debug=debug,
                          normalisation=normalisation)
    return train_datasets, val_datasets, bench_dataset
