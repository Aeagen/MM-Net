...
def pad_batch1_to_compatible_size(batch):
    print("input_before",batch.shape)
    shape = batch.shape
    zyx = list(shape[-3:])
    print("zyx",zyx)
    ...
    zmax, ymax, xmax = zyx
    print("xmax:{},ymax:{},xmax:{}".format(zmax,ymax,xmax))
    zpad, ypad, xpad = zmax - batch.size(2), ymax - batch.size(3), xmax - batch.size(4)  # original
    # zpad, ypad, xpad = zmax - batch.size(3), ymax - batch.size(4), xmax - batch.size(5)   # axial
    assert all(pad >= 0 for pad in (zpad, ypad, xpad)), "Negative padding value error !!"
