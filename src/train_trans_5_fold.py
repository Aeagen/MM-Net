...
parser.add_argument('--weight_decay', '--weight-decay', default=0., type=float,
...
parser.add_argument('--optims', action="store_true")
...
parser.add_argument('--fold', default=5, type=int, help="Split number (0 to 4)")
...
def main(args):
    ...
    if args.restore:
        ...
        #     4, 3,
        #     width=args.width, deep_supervision=args.deep_sup,
        #     norm_layer=None, dropout=args.dropout)
    ...
    if args.full:
        ...
        for i in range(0, args.fold):
            train_loader = torch.utils.data.DataLoader(
                train_dataset[i], batch_size=args.batch_size, shuffle=True,
                num_workers=args.workers, pin_memory=False, drop_last=True)
            val_loader = torch.utils.data.DataLoader(
                val_dataset[i], batch_size=max(1, args.batch_size // 2), shuffle=False,
                pin_memory=False, num_workers=args.workers)
            # val_loader = torch.utils.data.DataLoader(
        ...
        bench_loader = torch.utils.data.DataLoader(
            bench_dataset, batch_size=1, num_workers=args.workers)
        print("Val dataset number of batch:", len(val_loaders[1]))

    print("Train dataset number of batch:", len(train_loaders[1]))
    ...
    for i in range(0, args.fold):
        for epoch in range(args.start_epoch + args.warm, args.epochs + args.warm):
            try:
                ...
                ts = time.perf_counter()
                model.train()
                training_loss = step(train_loaders[i], model, criterion, metric, args.deep_sup, optimizer, epoch, t_writer,
                ...
                if (epoch + 1) % args.val == 0:
                    model.eval()
                    with torch.no_grad():
                        validation_loss = step_val(val_loaders[i], model, criterion, metric, args.deep_sup, optimizer,
                                               epoch,
                                               t_writer,
                                               save_folder=args.save_folder,
                                               no_fp16=args.no_fp16, patients_perf=patients_perf, set_kernal=False)
    ...
    if args.swa:
        ...
        for i in range(repeat):
            if args.optims:
                optimizer = torch.optim.SGD(model.parameters(), lr=args.lr / 2, weight_decay=args.weight_decay,
                                            momentum=0.9,
                                            nesterov=True)
            else:
            ...
            for swa_epoch in range(c):
                ...
                training_loss = step(train_loaders[swak], model, criterion, metric, args.deep_sup, optimizer,
                ...
                if (swa_epoch + 1) % k == 0:
                    ...
                    swa_model.eval()
                    with torch.no_grad():
                        validation_loss = step_val(val_loaders[swak], model, criterion, metric, args.deep_sup, optimizer,
                                               current_epoch,
                                               t_writer, save_folder=args.save_folder, no_fp16=args.no_fp16)
                        swa_model_loss = step_val(val_loaders[swak], swa_model, criterion, metric, args.deep_sup, optimizer,
                                              current_epoch,
